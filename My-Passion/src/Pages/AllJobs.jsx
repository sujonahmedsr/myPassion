...
const AllJobs = () => {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:5000/job?pages=${currentPage}&size=${itemsPerPage}&filter=${filter}&sort=${sort}`)
    .then(res => {
      setJobs(res.data);
    })
  ...
  return (
    <div className='container px-6 mx-auto min-h-[calc(100vh-306px)] flex flex-col justify-between py-20'>
      <div>
      ...
      <div className='flex justify-center mt-12'>
        <button className='px-4 py-2 mx-1 text-gray-700 disabled:text-gray-500 capitalize bg-gray-200 rounded-md disabled:cursor-not-allowed disabled:hover:bg-gray-200 disabled:hover:text-gray-500 hover:bg-blue-500  hover:text-white'>
          <div onClick={prevButton} className='flex items-center -mx-1'>
            <svg
        ...
        {pages.map(btnNum => (
          ...
          onClick={()=>setCurrentPage(btnNum)}
            key={btnNum}
            className={`hidden px-4 py-2 mx-1 transition-colors duration-300 transform  rounded-md sm:inline hover:bg-blue-500 ${currentPage === btnNum && 'bg-black text-white font-semibold'}  hover:text-white`}
          >
            {btnNum}
          </button>
        ))}
        ...
        <button className='px-4 py-2 mx-1 text-gray-700 transition-colors duration-300 transform bg-gray-200 rounded-md hover:bg-blue-500 disabled:hover:bg-gray-200 disabled:hover:text-gray-500 hover:text-white disabled:cursor-not-allowed disabled:text-gray-500'>
          <div onClick={nextButton} className='flex items-center -mx-1'>
